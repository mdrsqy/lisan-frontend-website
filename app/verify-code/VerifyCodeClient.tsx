...
export default function VerifyCodeClient() {
  ...
  const [timer, setTimer] = useState(60); // 60 detik cooldown
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);

  // Timer hitung mundur untuk kirim ulang
  useEffect(() => {
  ...
  const handleChange = (index: number, value: string) => {
    ...
    // Hanya ambil angka terakhir yang diketik
    if (value.length > 1) {
      ...
      setCode(newCode);
      // Focus ke digit terakhir yang terisi atau digit terakhir
      ...
      // Pindah ke input berikutnya jika ada isinya
      if (value && index < 5) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Biasanya perlu mengirim email juga (bisa diambil dari localStorage atau query params)
      ...
      <div className="w-full max-w-md px-4 relative z-10">
        ...
        <GlassCard className="p-8 md:p-10 rounded-[2.5rem]">
          <div className="mb-8 flex flex-col items-center text-center">
            <div className="w-16 h-16 rounded-2xl bg-blue-500/10 border border-blue-500/20 shadow-[0_0_30px_-5px_rgba(59,130,246,0.3)] flex items-center justify-center mb-6">
          ...
          <form onSubmit={handleSubmit}>
            <div className="flex justify-between gap-2 mb-8" onPaste={handlePaste}>
              {code.map((digit, index) => (
                <input
                  ...
                  className={`
                    w-12 h-14 md:w-14 md:h-16 rounded-xl bg-white/5 border text-center text-xl font-bold text-white 
                    focus:outline-none focus:ring-4 transition-all duration-200
                    ${digit 
                      ? "border-blue-500/50 shadow-[0_0_15px_-3px_rgba(59,130,246,0.3)] bg-blue-500/10" 
                      : "border-white/10 focus:border-blue-500/50 focus:bg-white/10 focus:ring-blue-500/10"
                    }
                  `}
                />
              ))}
            ...
            <div className="flex flex-col gap-4">
              <button
                ...
                className="w-full py-4 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 hover:scale-[1.02] active:scale-95 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm tracking-wide"
              >
